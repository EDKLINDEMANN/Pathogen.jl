function _count_by_state(events::Events{M},
                         state::DiseaseState,
                         time::Float64) where {S <: DiseaseStateSequence, M <: ILM{S}}
  if time < 0.0
    @error "Time must be ≥ 0.0"
  elseif state ∉ _state_progressions[S]
    @error "Invalid state specified"
  end
  n_ids = 0
  if state == _state_progressions[S][1] # S
  ...
  elseif state in _state_progressions[S][2:end-1]
  ...
  elseif state == _state_progressions[S][end] # I or R
